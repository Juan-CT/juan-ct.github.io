document.addEventListener('DOMContentLoaded', function () {
	...
	(function confBotones() {
		...
		const botonTop = document.querySelector('button.toTop');
	...
	function detectarCategoria(e) {
		...
		divTabla.innerHTML = divTabla.innerHTML !== "" ? "" : divTabla.innerHTML;
		const botonTop = document.querySelector('button.toTop');
		botonTop.classList.add('oculto');
	...
	function tratarDatos(datos) {
		...
		function pintarTablaDiez(datPag, pagina) {
			...
			const botonTop = document.querySelector('button.toTop');
			botonTop.classList.remove('oculto');
			...
			function crearTHead() {
				...
				if (siguiente.textContent !== "") {
					siguiente.addEventListener('click', () => {
						const nuevaPag = pagina + 1;
						repintarTablaDiez(datosPagina(nuevaPag), nuevaPag);
					});
				} else {
					siguiente.style.cursor = 'default';
				}
